...
func parseMmap(mmap *prog.Syscall, syscall *parser.Syscall, ctx *Context) *prog.Call {
	...
	addrArg, start := parseAddr(length, mmap.Args[0].Type, syscall.Args[0], ctx)
	lengthArg := prog.MakeConstArg(mmap.Args[1].Type, prog.DirIn, length)
	protArg := parseFlags(mmap.Args[2].Type, syscall.Args[2], ctx, false)
	flagArg := parseFlags(mmap.Args[3].Type, syscall.Args[3], ctx, true)
	fdArg := parseFd(mmap.Args[4].Type, syscall.Args[4], ctx)
	...
	call.Args = []prog.Arg{
		...
		flagArg,
		fdArg,
		prog.MakeConstArg(mmap.Args[5].Type, prog.DirIn, 0),
	}
...
func parseMremap(mremap *prog.Syscall, syscall *parser.Syscall, ctx *Context) *prog.Call {
	...
	ctx.CurrentSyzCall = call
	oldAddrArg, start := parseAddr(ctx.Target.PageSize, mremap.Args[0].Type, syscall.Args[0], ctx)
	oldSz := parseLength(syscall.Args[1], ctx)
	newSz := parseLength(syscall.Args[2], ctx)
	oldSzArg := prog.MakeConstArg(mremap.Args[1].Type, prog.DirIn, oldSz)
	newSzArg := prog.MakeConstArg(mremap.Args[2].Type, prog.DirIn, newSz)
	flags := parseFlags(mremap.Args[3].Type, syscall.Args[3], ctx, false)
	var destAddrArg prog.Arg
	var destAddr uint64
	if len(syscall.Args) > 4 {
		destAddrArg, destAddr = parseAddr(ctx.Target.PageSize, mremap.Args[4].Type, syscall.Args[4], ctx)
	} else {
		straceAddrArg := parser.NewIntsType([]int64{syscall.Ret})
		destAddrArg, destAddr = parseAddr(ctx.Target.PageSize, mremap.Args[4].Type, straceAddrArg, ctx)
	}
	...
	addDependency(destAddr, newSz, destAddrArg, ctx)
	return call
}
...
func parseMsync(msync *prog.Syscall, syscall *parser.Syscall, ctx *Context) *prog.Call {
	...
	ctx.CurrentSyzCall = call
	addrArg, address := parseAddr(ctx.Target.PageSize, msync.Args[0].Type, syscall.Args[0], ctx)
	length := parseLength(syscall.Args[1], ctx)
	lengthArg := prog.MakeConstArg(msync.Args[1].Type, prog.DirIn, length)
	protArg := parseFlags(msync.Args[2].Type, syscall.Args[2], ctx, false)
...
func parseMprotect(mprotect *prog.Syscall, syscall *parser.Syscall, ctx *Context) *prog.Call {
	...
	ctx.CurrentSyzCall = call
	addrArg, address := parseAddr(ctx.Target.PageSize, mprotect.Args[0].Type, syscall.Args[0], ctx)
	length := parseLength(syscall.Args[1], ctx)
	lengthArg := prog.MakeConstArg(mprotect.Args[1].Type, prog.DirIn, length)
	protArg := parseFlags(mprotect.Args[2].Type, syscall.Args[2], ctx, false)
	addDependency(address, length, addrArg, ctx)
	call.Args = []prog.Arg{
		addrArg,
		lengthArg,
...
func parseMunmap(munmap *prog.Syscall, syscall *parser.Syscall, ctx *Context) *prog.Call {
	...
	ctx.CurrentSyzCall = call
	addrArg, address := parseAddr(ctx.Target.PageSize, munmap.Args[0].Type, syscall.Args[0], ctx)
	length := parseLength(syscall.Args[1], ctx)
	lengthArg := prog.MakeConstArg(munmap.Args[1].Type, prog.DirIn, length)
	addDependency(address, length, addrArg, ctx)
	call.Args = []prog.Arg{
		addrArg,
		lengthArg,
...
func parseMadvise(madvise *prog.Syscall, syscall *parser.Syscall, ctx *Context) *prog.Call {
	...
	ctx.CurrentSyzCall = call
	addrArg, address := parseAddr(ctx.Target.PageSize, madvise.Args[0].Type, syscall.Args[0], ctx)
	length := parseLength(syscall.Args[1], ctx)
	lengthArg := prog.MakeConstArg(madvise.Args[1].Type, prog.DirIn, length)
	var adviceArg prog.Arg
	switch a := syscall.Args[2].(type) {
	case parser.Expression:
		adviceArg = prog.MakeConstArg(madvise.Args[2].Type, prog.DirIn, a.Eval(ctx.Target))
	default:
		log.Fatalf("Failed to parse advice arg for madvise")
	}
	addDependency(address, length, addrArg, ctx)
	call.Args = []prog.Arg{
		addrArg,
...
func parseMlock(mlock *prog.Syscall, syscall *parser.Syscall, ctx *Context) *prog.Call {
	...
	ctx.CurrentSyzCall = call
	addrArg, address := parseAddr(ctx.Target.PageSize, mlock.Args[0].Type, syscall.Args[0], ctx)
	length := parseLength(syscall.Args[1], ctx)
	flagArg := prog.MakeConstArg(mlock.Args[1].Type, prog.DirIn, length)
	addDependency(address, length, addrArg, ctx)
	call.Args = []prog.Arg{
		addrArg,
		flagArg,
...
func parseMunlock(munlock *prog.Syscall, syscall *parser.Syscall, ctx *Context) *prog.Call {
	...
	ctx.CurrentSyzCall = call
	addrArg, address := parseAddr(ctx.Target.PageSize, munlock.Args[0].Type, syscall.Args[0], ctx)
	length := parseLength(syscall.Args[1], ctx)
	flagArg := prog.MakeConstArg(munlock.Args[1].Type, prog.DirIn, length)
	addDependency(address, length, addrArg, ctx)
	call.Args = []prog.Arg{
		addrArg,
		flagArg,
...
func parseShmat(shmat *prog.Syscall, syscall *parser.Syscall, ctx *Context) *prog.Call {
	...
	* store it at that address if successful.
	...
	if arg := ctx.ReturnCache.get(shmat.Args[0].Type, syscall.Args[0]); arg != nil {
		switch a := arg.Type().(type) {
		case *prog.ResourceType:
			fd = prog.MakeResultArg(shmat.Args[0].Type, prog.DirIn, arg.(*prog.ResultArg), a.Default())
		default:
		...
		fd = prog.MakeResultArg(shmat.Args[0].Type, prog.DirIn, nil, shmid)
	}

	addrArg, address := parseAddr(ctx.Target.PageSize, shmat.Args[1].Type, syscall.Args[1], ctx)
	flags := parseFlags(shmat.Args[2].Type, syscall.Args[2], ctx, false)
...
func parseAddr(length uint64, syzType prog.Type, traceType parser.IrType, ctx *Context) (prog.Arg, uint64) {
	...
	case *parser.PointerType:
		...
		if a.IsNull() {
			...
			return prog.MakeVmaPointerArg(syzType, prog.DirIn, defAddrStart, length), addrStart
		}
		return prog.MakeVmaPointerArg(syzType, prog.DirIn, defAddrStart, length), a.Address
	case parser.Expression:
...
func parseFlags(syzType prog.Type, straceType parser.IrType, ctx *Context, mapFlag bool) prog.Arg {
	...
	case parser.Expression:
		if mapFlag {
			...
			return prog.MakeConstArg(syzType, prog.DirIn, val)
		}
		return prog.MakeConstArg(syzType, prog.DirIn, a.Eval(ctx.Target))
...
func parseFd(syzType prog.Type, straceType parser.IrType, ctx *Context) prog.Arg {
	...
	if arg := ctx.ReturnCache.get(syzType, straceType); arg != nil {
		...
		case *prog.ResourceType:
			return prog.MakeResultArg(arg.Type(), prog.DirIn, arg.(*prog.ResultArg), a.Default())
		default:
	...
	switch a := straceType.(type) {
	case parser.Expression:
		return prog.MakeResultArg(syzType, prog.DirIn, nil, a.Eval(ctx.Target))
	default:
